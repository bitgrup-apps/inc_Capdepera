...
function error_(codi, json, error) {
    $.post('http://www.arta.cat:82/App/incidencies.php', {funcio: 'appError', TOKEN: 'LAIDSD88347ERJKADKFGKAHPF8YA9DF8Y', codi: codi, json: json, error: error}, function (data) {});
}
...
function enviaIncidencia() {
    ...
    if (window.numImatges == 0) {
        $.ajax({
            url: 'http://www.arta.cat:82/App/incidencies.php', type: 'POST', data: parametros,
        ...
        ft.upload(imageURI, encodeURI("http://www.arta.cat:82/App/incidencies.php"), win, fail, options);
    }
...
function enviaSegonaImatge(id) {
    ...
    ft.upload(imageURI, encodeURI("http://www.arta.cat:82/App/incidencies.php"), win2, fail2, options);
}
...
function enviaTerceraImatge(id) {
    ...
    ft.upload(imageURI, encodeURI("http://www.arta.cat:82/App/incidencies.php"), win3, fail2, options);
}
...
function comprovaPosicio(lat, long) {
    var ok = true;
    //limits regions artà
    if (lat > 39.787776 || lat < 39.629692 || long < 3.236807 || long > 3.389754)
        ok = false;
    return ok;
}
function errorMapa() {
